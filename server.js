...
const multer = require('multer'); // Assuming you're handling file uploads
...
// Example crafts data integrated directly into the server
let crafts = [
...
app.get("/api/crafts", (req, res) => {
    // Send the crafts array as a response
    res.json(crafts);
});

app.post("/api/add-craft", upload.single('img'), (req, res) => {
    // Example of adding a new craft (simplified for brevity)
    const newCraft = {
        name: req.body.name,
        description: req.body.description,
        supplies: req.body.supplies.split(','), // Assuming supplies are sent as a comma-separated string
        img: req.file.path // Path where the uploaded image is stored
    };

    crafts.push(newCraft); // Add the new craft to the array
